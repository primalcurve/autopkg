...
import appdirs
from autopkglib.common import VarDict, is_mac, log_err
...
class Preferences:
    ...
    def __init__(self):
        ...
        # macOS: check for a default preferences file first
        # if empty, and no --prefs passed in, check for preferences domain
        # only use preferences domain on macOS if no --prefs passed in
        if is_mac():
            self.prefs = self._get_macos_prefs()
        else:
            self.prefs = self._get_file_prefs()
            # On non-macOS, if the file prefs don't exist, create a blank one
            if not self.prefs:
